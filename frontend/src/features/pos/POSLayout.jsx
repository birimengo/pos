...
export default function POSLayout() {
  ...
  const navigation = [
    ...
    // New items - Invoicing, Quotations, Suppliers
  ...
  return (
    <div className={`flex h-screen ${currentTheme.colors.bg} transition-colors duration-300 overflow-hidden`}>
      ...
      >
        ...
        {/* Navigation - Scrollable with compact sizing */}
      ...
      <div className="flex-1 flex flex-col overflow-hidden w-full">
        {/* Header - Ultra Compact with specific heading color */}
        <header className={`h-10 ${currentTheme.colors.header} border-b ${currentTheme.colors.border} flex items-center justify-between px-2 flex-shrink-0`}>
          ...
          <div className="flex items-center gap-0.5">
            ...
            {state.stores && state.stores.length > 1 && !isMobile && (
              <select
                className={`px-1.5 py-1 text-[9px] rounded border ${currentTheme.colors.border} ${currentTheme.colors.bgSecondary} ${currentTheme.colors.text} max-w-[90px]`}
              ...
              >
                {state.stores.map(store => (
                  <option key={store.id} value={store.id}>{store.name.substring(0, 8)}</option>
                ))}
              </select>
            )}
