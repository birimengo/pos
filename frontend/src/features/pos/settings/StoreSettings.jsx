...
export default function StoreSettings({ currentStore: propCurrentStore, onStoreUpdate }) {
  ...
  const { updateStoreSettings, refreshSettings } = useSettings();
  ...
  // Load current store - ALWAYS use activeStore from context
  useEffect(() => {
    loadCurrentStore();
  }, [activeStore]);
  ...
  const loadCurrentStore = async () => {
    ...
    try {
      ...
      if (activeStore) {
        store = activeStore;
        console.log('📦 Using activeStore from context:', store.name, 'ID:', store._id);
      } 
      // Priority 2: Get from database using currentStoreId
      else {
        const activeStoreId = db.getCurrentStore();
        if (activeStoreId) {
          store = await db.getStore(activeStoreId);
          if (store) {
      ...
      // Priority 3: Get default store from API
      if (!store) {
      ...
      if (store) {
        ...
        if (store.createdBy === user?.id) {
          ...
          console.error('❌ Store does not belong to current user:', store.name);
          // Try to get the user's first store
          const userStores = await api.getAllStores();
          if (userStores.success && userStores.stores?.length > 0) {
            const userStore = userStores.stores[0];
            setCurrentStore(userStore);
            loadFormFromStore(userStore);
            console.log('📦 Using first user store:', userStore.name);
          } else {
            setCurrentStore(null);
          }
        }
      } else {
        console.warn('⚠️ No store found to edit');
        setCurrentStore(null);
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-3 pt-4 border-t">
          <button
            type="button"
            onClick={() => loadFormFromStore(currentStore)}
